...
# 注意：$Profile 是 effect 容器 profile。具体 pixel shader profile 请在 .fx 的 pass 内写
#       例如 PixelShader = compile ps_3_0 PixelShaderFunction();

param(
    [string]$Dir = $PSScriptRoot,
    [string]$Compiler = "C:\Users\Hommeng\Documents\My Games\Terraria\tModLoader\FXC\fxc.exe",
    [string]$Profile = "fx_2_0"
)
...
foreach ($fx in $fxFiles) {
    ...
    Write-Host "[编译] $($fx.Name) -> $baseName.fxc ($Profile)" -ForegroundColor Cyan -NoNewline
    $output = & $Compiler /T $Profile /Fo $fxcPath $fx.FullName 2>&1
